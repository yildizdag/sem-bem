function semOpt2D = semOpt2Dmesh(Nurbs2D_plate,Nurbs2D_stiff,N,shell_dof)
nel = 0;
for k = 1:Nurbs2D_plate.numpatch
    nel = nel + Nurbs2D_plate.nel{k};
end
for k = 1:Nurbs2D_stiff.numpatch
    nel = nel + Nurbs2D_stiff.nel{k};
end
semOpt2D.nel = nel;
semOpt2D.N = N;
...
for k = 1:Nurbs2D_plate.numpatch
    for el = 1:Nurbs2D_plate.nel{k}
        ...
        count = 1;
        CP = Nurbs2D_plate.cPoints{k}(:,iu-Nurbs2D_plate.order{k}(1)+1:iu, iv-Nurbs2D_plate.order{k}(2)+1:iv);
        du = (Nurbs2D_plate.knots.U{k}(iu+1)-Nurbs2D_plate.knots.U{k}(iu))/2;
        dv = (Nurbs2D_plate.knots.V{k}(iv+1)-Nurbs2D_plate.knots.V{k}(iv))/2;
        for i = 1:N
            for j = 1:N
                dNu = dersbasisfuns(iu,u_sample(i),Nurbs2D_plate.order{k}(1)-1,2,Nurbs2D_plate.knots.U{k});
                dNv = dersbasisfuns(iv,v_sample(j),Nurbs2D_plate.order{k}(2)-1,2,Nurbs2D_plate.knots.V{k});
                [~,dS] = derRat2DBasisFuns(dNu,dNv,Nurbs2D_plate.order{k}(1),Nurbs2D_plate.order{k}(2),CP,2,2);
...
for k = 1:Nurbs2D_stiff.numpatch
    for el = 1:Nurbs2D_stiff.nel{k}
        ...
        v_sample = (0.5.*(1-eta).*v1+0.5.*(1+eta).*v2);
        count = 1;
        CP = Nurbs2D_stiff.cPoints{k}(:,iu-Nurbs2D_stiff.order{k}(1)+1:iu, iv-Nurbs2D_stiff.order{k}(2)+1:iv);
        du = (Nurbs2D_stiff.knots.U{k}(iu+1)-Nurbs2D_stiff.knots.U{k}(iu))/2;
        dv = (Nurbs2D_stiff.knots.V{k}(iv+1)-Nurbs2D_stiff.knots.V{k}(iv))/2;
        for i = 1:N
            for j = 1:N
                dNu = dersbasisfuns(iu,u_sample(i),Nurbs2D_stiff.order{k}(1)-1,2,Nurbs2D_stiff.knots.U{k});
                dNv = dersbasisfuns(iv,v_sample(j),Nurbs2D_stiff.order{k}(2)-1,2,Nurbs2D_stiff.knots.V{k});
                [~,dS] = derRat2DBasisFuns(dNu,dNv,Nurbs2D_stiff.order{k}(1),Nurbs2D_stiff.order{k}(2),CP,2,2);
                nodeData(count_node,:) = epsilon.*(dS(:,1,1)'./epsilon);
                %
                A1 = dS(:,2,1); A2 = dS(:,1,2);
                t1 = A1 ./ norm(A1);
