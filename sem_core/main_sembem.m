...
%
countBEM = size(sembem.nodesBEM,1);
%
...
dist_tol = 0.18;
%------------------------------------
[N, dN] = shapefunc2D(xgp(:,1),xgp(:,2),sembem2D.N);
count_col = 1;

for j=1:size(sembem2D.nodesBEM,1)

    node_i = sembem.nodesBEM(j,:);
    node_ip = [node_i(1), -node_i(2), node_i(3)];

    ni = sembem.nBEM(j,:);
    ind = find((abs(sembem.nodes(:,1)-node_i(1))<1E-6) & (abs(sembem.nodes(:,2)-node_i(2)))<1E-6 & (abs(sembem.nodes(:,3)-node_i(3)))<1E-6);
    if ~isempty(ind)
        b(count_col,:) = -ni(1).*U_ModesX(ind,:)-ni(2).*U_ModesY(ind,:)-ni(3).*U_ModesZ(ind,:);
    end
    addDOF = 0;
