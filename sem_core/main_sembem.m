...
V = V(:,loc);
freqHz = freq/2/pi;
freqND = freq.*10^2/2/pi*sqrt(sembem2D.rho*sembem2D.t/sembem2D.D);
%
...
[wV, wfreq] = eig(c,(a+A));
wfreq = diag(wfreq);
[wfreq2,ind] = sort((sqrt(real(wfreq))));
wfreqND = wfreq2.*10^2/2/pi*sqrt(sembem2D.rho*sembem2D.t/sembem2D.D);