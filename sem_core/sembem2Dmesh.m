...
%JacMatDataBEM = zeros(2,2,ntotBEM);
%InvJacMatDataBEM = zeros(2,2,ntotBEM);
%JacobianDataBEM = zeros(ntotBEM,1);
%curvDataBEM = zeros(ntotBEM,2);
%
%T1Data = zeros(ntot,3);   % local tangent-1 (global components)
%T2Data = zeros(ntot,3);   % local tangent-2 (global components)
NDataBEM  = zeros(ntotBEM,3);   % local normal    (global components)
%RData  = zeros(3,3,ntot); % optional: rotation matrix per sampling point
%
count_el = 1;
count_node = 1;
